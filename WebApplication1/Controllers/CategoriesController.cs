...
namespace WebApplication1.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        [HttpGet]
        public IActionResult GetByWords(Guid id, string words)
        {
            var selected = new SelectedCategoryModel();
            selected.CategoryId = id;
            selected.Category = _context.Categories
                .First(r => r.CategoryId == id).Name;
            
            selected.Products = _context.Products
                .Where(t =>
                    t.Name.StartsWith(words) || 
                    t.Name.Contains(words) || 
                    t.Name.EndsWith(words)).ToList();
